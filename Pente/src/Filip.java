...
public Square filsNextMove(int check){
	if(myBoard.winY(theGameBoard[LMR][LMC],false)>=check){
		int rowI = 1;
	...
	if(myBoard.winX(theGameBoard[LMR][LMC], false)>=check){
		int colI = 1;
		...
		if(theGameBoard[LMR][LMC+colI].getState()==PenteMain.EMPTY){
			...
			colI=1;
			while(theGameBoard[LMR][LMC-colI].getState()== theGameBoard[LMR][LMC].getState() 
					&& theGameBoard[LMR][LMC-colI].getState() > 0){
				colI++;
			}
			if(theGameBoard[LMR][LMC-colI].getState()==PenteMain.EMPTY){
				return theGameBoard[LMR][LMC-colI];
	...
	if(myBoard.winD1(theGameBoard[LMR][LMC],false)>=check){
		int rowI = 1;
	...
	if(myBoard.winD2(theGameBoard[LMR][LMC],false)>=check){
		int colI = 1;
...
public Square offense(int chek){
		...
		if(best >= chek){
			if(myBoard.winY(black.get(bi),false)== best &&myBoard.winX(black.get(bi),false)==best&&myBoard.winD1(black.get(bi),false)==best&&myBoard.winD2(black.get(bi),false)==best){
		...
		else if(myBoard.winY(black.get(bi),false) == getBestLine(black.get(bi))){
				return placeY(black.get(bi), bi, chek);		
			}if(myBoard.winX(black.get(bi),false) == getBestLine(black.get(bi))){
				return placeX(black.get(bi), bi, chek);
			}if(myBoard.winD1(black.get(bi),false) == getBestLine(black.get(bi))){
				return placeD1(black.get(bi), bi, chek);
			}		
				return placeD2(black.get(bi), bi, chek);
	...
	public int getBestLine(Square s){
		int bestNum = myBoard.winY(s, false);
		if(myBoard.winX(s, false) > bestNum){
			bestNum = myBoard.winX(s, false);
		}if(myBoard.winD1(s, false) > bestNum){
			bestNum = myBoard.winD1(s, false);
		}if(myBoard.winD2(s,false) > bestNum){
			bestNum = myBoard.winD2(s,false);
		}
